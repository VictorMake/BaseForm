...
Friend Class FormCalculatedParameters
    ...
    Public Sub ConfigureTableAdapter()
        ...
        CalculatedParametersTableAdapter.Connection = New OleDbConnection(BuildCnnStr(PROVIDER_JET, mFormParrent.Manager.PathSettingMdb))

        'This line of code loads data into the 'BaseFormDataSet.РасчетныеПараметры' table. You can move, or remove it, as needed.
        CalculatedParametersTableAdapter.Fill(BaseFormDataSet.РасчетныеПараметры)
    End Sub
    ...
    Private Sub TSButtonHelp_Click(ByVal sender As Object, ByVal e As EventArgs) Handles TSButtonHelp.Click
        MessageBox.Show($"Если в формуле расчёта используются единицы СИ, то для каждого расчётного параметра установить <Выходную единицу измерения>,{Environment.NewLine}в противном случае установить <нет>.",
...
'    arrИменаПараметров(0) = ПАРАМЕТР_ОТСУТСТВУЕТ
...
'    ReDim_arrЕдИзмерения(Convert.ToInt32(cmd.ExecuteScalar))
...
'        arrСоответствие(I).NРасчета = CShort(DGVСоответсвие.Rows(I - 1).Cells("ColumnНомерПараметраВРасчете").Value) 'CShort(F1View.TextSRC(1, I + conRowOffset, 3))
...
'        arrСоответствие(I).NБазы = CShort(DGVСоответсвие.Rows(I - 1).Cells("ColumnНомерПараметраИзмерения").Value) 'CShort(F1View.TextSRC(1, I + conRowOffset, 5))
...
'        If MessageBox.Show("Были произведены изменения в таблице свойств." & Environment.NewLine & "Записать измерения ?", "Соответствие", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) = Windows.Forms.DialogResult.Yes Then
